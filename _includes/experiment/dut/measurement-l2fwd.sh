...
if [ ! $IRQRECORDING -eq 0 ]; then
	...
	pos_run -l irql2fwd -- taskset -c 3 /root/experiment-script/irq/irqrecorder.py
	sleep 2
else
	echo "no irq recording"
fi

pos_run -l l2fwdrun -- bash -c "/root/dpdk-21.11/build/examples/dpdk-l2fwd -l 1 -n 4 -- -q 3 -p 0x3"
sleep 30
