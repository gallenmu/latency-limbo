...
import() {
	...
	local name="$2"
        ...
        [[ "$prepcap" == *-pre.pcap_run* ]] || [[ "$prepcap" == *pre.pcap_run*.zst ]] || err input \""$prepcap"\" does not match expected pattern
	...
	test -r "$postpcap" || err can not read post cap: \""$postpcap"\"
        ...
        # unzip if needed
        remove=0
        if [[ "$prepcap" == *pre.pcap_run*.zst ]]; then
        ...
        if [[ "$postpcap" == *post.pcap_run*.zst ]]; then
